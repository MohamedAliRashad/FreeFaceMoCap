...
def add_group_empty(empties):
    num = len(empties)
    location = sum([empty.matrix_world.translation for empty in empties if not empty.name.endswith('_head')], Vector()) / num
    ...
    if not get_name_of_object(Config.group_empty_prefix) in bpy.data.objects:
        ...
        group_empty.scale = tuple(Config.group_empty_initial_scale for _ in range(3))

    else:
        group_empty = bpy.data.objects[get_name_of_object(Config.group_empty_prefix)]
